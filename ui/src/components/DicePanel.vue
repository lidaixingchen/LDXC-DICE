...
function selectCombatTarget(name: string): void {
  ...
  if (char) {
    ...
    if (attrs['DDC'] !== undefined) {
      targetDDC.value = attrs['DDC'];
    }
    if (attackType.value === '物理' && attrs['物理防御'] !== undefined) {
      targetDefense.value = attrs['物理防御'];
    } else if (attackType.value === '法术' && attrs['法术防御'] !== undefined) {
      targetDefense.value = attrs['法术防御'];
    } else if (attrs['防御'] !== undefined) {
      targetDefense.value = attrs['防御'];
    }
  }
}
...
watch(attackType, () => {
  if (selectedTarget.value) {
    selectCombatTarget(selectedTarget.value);
  }
});
...
function startCombat(): void {
  ...
  if (selectedTarget.value) {
    const targetChar = characters.value.find(c => c.name === selectedTarget.value);
    if (targetChar) {
...
${combat.value.round >= 6 ? '⚠️ 环境侵蚀生效！每回合承受 ' + Math.floor(combat.value.playerMaxHP * 0.05) + ' 点真实伤害' : ''}
</meta:战斗状态>`;
  await sendToTextarea(content);
}
...
function deriveCombatStats(attrs: Record<string, number>, level: string): DerivedStats {
  ...
  const hpBase = WORLD_LEVEL_CONFIG[level]?.hpBase || 15;
...
async function handleCombatCheck(): Promise<void> {
  ...
  if (isHit && combat.value.active) {
    const oldHP = combat.value.enemyCurrentHP;
    const newHP = Math.max(0, oldHP - finalDamage);
    combat.value.enemyCurrentHP = newHP;
    
    combatStatusContent = `
<meta:战斗状态>
【第${combat.value.round}回合 - 玩家攻击】

⚔️ ${atkTypeLabel}${isCrit ? '暴击' : ''}命中${targetName}！
・造成伤害：${finalDamage}点${isCrit ? '（暴击x2）' : ''}
...
async function handleDefenseCheck(): Promise<void> {
  ...
  const attackerName = selectedDefenseTarget.value || '敌人';
  ...
  if (!isDodge && combat.value.active) {
    ...
    if (combat.value.playerShield > 0) {
      if (combat.value.playerShield >= baseDamage) {
        combat.value.playerShield -= baseDamage;
        actualDamage = 0;
      } else {
        actualDamage = baseDamage - combat.value.playerShield;
        combat.value.playerShield = 0;
      }
    }
    
    const oldHP = combat.value.playerCurrentHP;
    const newHP = Math.max(0, oldHP - actualDamage);
    combat.value.playerCurrentHP = newHP;
    
    combatStatusContent = `
<meta:战斗状态>
【第${combat.value.round}回合 - 敌方攻击】
      ...
      <div v-if="checkMode === 'combat' && !isCustomMode" class="acu-section-card">
        ...
        <div v-if="npcList.length > 0" class="acu-target-selector">
          ...
          <div class="acu-target-buttons">
            <button
              ...
              @click="selectCombatTarget(npc.name)"
            >
        ...
        <div class="acu-dice-form-row cols-3">
          ...
          <div class="acu-dice-field">
            ...
            <input v-model="targetDDC" type="text" class="acu-dice-input" :placeholder="selectedTarget ? '已自动填充' : '闪避难度'" />
          </div>
          <div class="acu-dice-field">
            <div class="acu-dice-form-label">目标防御</div>
            <input v-model="targetDefense" type="text" class="acu-dice-input" :placeholder="selectedTarget ? '已自动填充' : '伤害计算用'" />
          </div>
        </div>
      ...
      <div v-if="checkMode === 'defense' && !isCustomMode" class="acu-section-card">
        ...
        <div v-if="npcList.length > 0" class="acu-target-selector">
          ...
          <div class="acu-target-buttons">
            <button
              ...
              @click="selectDefenseTarget(npc.name)"
            >
              {{ npc.name.length > 4 ? npc.name.slice(0, 4) + '..' : npc.name }}
            </button>
          </div>
        ...
        <div class="acu-dice-form-row cols-3">
          <div class="acu-dice-field">
            ...
            <input v-model="agilityValue" type="text" class="acu-dice-input" :placeholder="selectedDefenseTarget ? '已自动填充' : '敏捷属性'" />
          </div>
          <div class="acu-dice-field">
            <div class="acu-dice-form-label">感知值</div>
            <input v-model="perceptionValue" type="text" class="acu-dice-input" :placeholder="selectedDefenseTarget ? '已自动填充' : '感知属性'" />
          </div>
          <div class="acu-dice-field">
      ...
      <div v-if="checkMode === 'initiative' && !isCustomMode" class="acu-section-card">
        ...
        <div class="acu-dice-form-row cols-3">
          <div class="acu-dice-field">
            ...
            <input v-model="attrValue" type="text" class="acu-dice-input" placeholder="敏捷属性值" />
          </div>
          <div class="acu-dice-field">
            <div class="acu-dice-form-label">对方敏捷</div>
            <input v-model="oppAgility" type="text" class="acu-dice-input" placeholder="对方敏捷值" />
          </div>
        </div>
      ...
      <div v-if="checkMode === 'escape' && !isCustomMode" class="acu-section-card">
        ...
        <div v-if="escapeType === 'surrounded'" class="acu-dice-form-row cols-3">
          <div class="acu-dice-field">
            ...
            <input v-model="escapeEnemyAgility" type="text" class="acu-dice-input" placeholder="影响DC" />
          </div>
          <div class="acu-dice-field">
            <div class="acu-dice-form-label">敌人数量</div>
            <input v-model="escapeEnemyCount" type="text" class="acu-dice-input" placeholder="每人+2 DC" />
          </div>
        </div>
        ...
        <div v-if="escapeType === 'obstacle'" class="acu-dice-form-row cols-3">
          ...
          <div class="acu-dice-field">
            ...
            <input v-model="escapeObstacleMod" type="text" class="acu-dice-input" placeholder="障碍加成" />
          </div>
        </div>
      ...
      <div class="acu-combat-manager acu-section-card" v-if="!isCustomMode && (checkMode === 'combat' || checkMode === 'defense' || checkMode === 'initiative')">
        <div class="acu-card-header">
          ...
          <template v-else>
            ...
            <button class="acu-tiny-btn" @click="sendCombatStatus" title="发送状态">📤 状态</button>
