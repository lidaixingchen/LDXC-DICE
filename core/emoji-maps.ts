export type IconMap = { pattern: RegExp; icon: string } | [RegExp, string];

export interface RelationIconMapping {
  icon: string;
  keywords: string[];
}

export const EMOJI_MAP: IconMap[] = [
  [/荞麦面|拉面/, 'ti:soup'],
...
export function getIconForText(text: string): string | null {
  for (const item of EMOJI_MAP) {
    ...
    const icon = Array.isArray(item) ? item[1] : item.icon;
...
export function getRelationIcon(relationText: string): string | null {
  const lowerText = relationText.toLowerCase();
  for (const { icon, keywords } of RELATION_ICON_MAP) {
